...
class dfe:
    ...
    def sslms(self, ekin):
        ...
        dfe_tap[0] = 1. / 64. * 0.64*(self.htaps[0][-1]*64/0.64 + 1. / 8 * delta * self.ekin * (
        np.sign(self.din[-2] + 2) ))#-1 * np.sign(self.ekin) * np.sign(self.din[-2]) + 2) / 8

        dfe_tap[1] = self.htaps[1][-1] + 1. / 16 * delta * np.sign(self.ekin) * np.sign(self.din[-2]) / 64. * 0.64
        dfe_tap[2] = self.htaps[2][-1] + 1. / 32 * delta * np.sign(self.ekin) * np.sign(self.din[-3]) / 64. * 0.32
        dfe_tap[3] = self.htaps[3][-1] + 1. / 64 * delta * np.sign(ekin) * np.sign(self.din[-4]) / 64. * 0.32
        dfe_tap[4] = self.htaps[4][-1] + 1. / 128 * delta * np.sign(ekin) * np.sign(self.din[-5]) / 64. * 0.16
        dfe_tap[5] = self.htaps[5][-1] + 1. / 128 * delta * np.sign(ekin) * np.sign(self.din[-6]) / 64. * 0.16
        dfe_tap[6] = self.htaps[6][-1] + 1. / 128 * delta * np.sign(ekin) * np.sign(self.din[-7]) / 64. * 0.16
        dfe_tap[7] = self.htaps[7][-1] + 1. / 128 * delta * np.sign(ekin) * np.sign(self.din[-8]) / 64 * 0.16
